...
void Ssao::RebuildDescriptors(ID3D12Resource* depthStencilBuffer)
{
    ...
    UINT index = DescriptorManager::GetInstance().CreateSRV_CBV_UAV(mNormalMap, srvDesc);
    mhNormalMapCpuSrv = DescriptorManager::GetInstance().GetSRVDescriptorHandle_CPU(index);
    mhNormalMapGpuSrv= DescriptorManager::GetInstance().GetSRVDescriptorHandle_GPU(index);


    srvDesc.Format = DXGI_FORMAT_R24_UNORM_X8_TYPELESS;
    //md3dDevice->CreateShaderResourceView(depthStencilBuffer, &srvDesc, mhDepthMapCpuSrv);

    index = DescriptorManager::GetInstance().CreateSRV_CBV_UAV(depthStencilBuffer, srvDesc);
    mhDepthMapCpuSrv = DescriptorManager::GetInstance().GetSRVDescriptorHandle_CPU(index);
    mhDepthMapGpuSrv= DescriptorManager::GetInstance().GetSRVDescriptorHandle_GPU(index);
    srvDesc.Format = DXGI_FORMAT_R8G8B8A8_UNORM;
    //md3dDevice->CreateShaderResourceView(mRandomVectorMap.Get(), &srvDesc, mhRandomVectorMapCpuSrv);
    index = DescriptorManager::GetInstance().CreateSRV_CBV_UAV(mRandomVectorMap, srvDesc);
    mhRandomVectorMapCpuSrv = DescriptorManager::GetInstance().GetSRVDescriptorHandle_CPU(index);
    mhRandomVectorMapGpuSrv = DescriptorManager::GetInstance().GetSRVDescriptorHandle_GPU(index);
   ...
   // md3dDevice->CreateShaderResourceView(mAmbientMap0.Get(), &srvDesc, mhAmbientMap0CpuSrv);
    ...
    index = DescriptorManager::GetInstance().CreateSRV_CBV_UAV(mAmbientMap0, srvDesc);
    mhAmbientMap0CpuSrv = DescriptorManager::GetInstance().GetSRVDescriptorHandle_CPU(index);
    mhAmbientMap0GpuSrv= DescriptorManager::GetInstance().GetSRVDescriptorHandle_GPU(index);
    index = DescriptorManager::GetInstance().CreateSRV_CBV_UAV(mAmbientMap1, srvDesc);
    mhAmbientMap1CpuSrv = DescriptorManager::GetInstance().GetSRVDescriptorHandle_CPU(index);
    mhAmbientMap1GpuSrv= DescriptorManager::GetInstance().GetSRVDescriptorHandle_GPU(index);
    ...
    index = DescriptorManager::GetInstance().CreateRtv(mNormalMap, rtvDesc);
    mhNormalMapCpuRtv = DescriptorManager::GetInstance().GetRTVDescriptorHandle_CPU(index);
    ...
    index = DescriptorManager::GetInstance().CreateRtv(mAmbientMap0, rtvDesc);
    mhAmbientMap0CpuRtv = DescriptorManager::GetInstance().GetRTVDescriptorHandle_CPU(index);
    index = DescriptorManager::GetInstance().CreateRtv(mAmbientMap1, rtvDesc);
    mhAmbientMap1CpuRtv = DescriptorManager::GetInstance().GetRTVDescriptorHandle_CPU(index);
}
    ...
    int blurCount)
{
    ...
    cmdList->SetGraphicsRootDescriptorTable(4, DescriptorManager::GetInstance().GetSRVDescriptorHandle_GPU(TextureMgr::GetInstance().GetTexture(L"../Textures/random.dds").SRVheapIndex));
...
{
    TextureMgr::GetInstance().CreateUploadTexture(TextureStyle::DiffuseTexture, L"../Textures/random.dds");
    //D3D12_RESOURCE_DESC texDesc;
    //ZeroMemory(&texDesc, sizeof(D3D12_RESOURCE_DESC));
    //texDesc.Dimension = D3D12_RESOURCE_DIMENSION_TEXTURE2D;
    //texDesc.Alignment = 0;
